...
final class FavoritesViewModel {
    ...
    func items(for section: FavoriteSection) -> Int {
        switch section {
        case .books:
            return favoriteEbooks.count
        case .audiobooks:
            return favoriteBooks.count
        case .kids:
            return favoriteKidsBooks.count
        case .authors:
    ...
    func toggleKidsBook(_ ebook: Ebook) {
        if let idx = favoriteKidsBooks.firstIndex(where: { $0.id == ebook.id }) {
            ...
            favoriteKidsBooks.insert(ebook,
                                     at: 0)
        }
        saveToLocal()
        syncToFirebase()
...
private extension FavoritesViewModel {
    ...
    func saveToLocal() {
        if let d = try? JSONEncoder().encode(favoriteBooks) { UserDefaults.standard.set(d,
                                                                                        forKey: booksKey) }
        if let d = try? JSONEncoder().encode(favoriteEbooks) { UserDefaults.standard.set(d,
                                                                                         forKey: ebooksKey) }
        if let d = try? JSONEncoder().encode(favoriteKidsBooks) { UserDefaults.standard.set(d,
                                                                                            forKey: kidsKey) }
        if let d = try? JSONEncoder().encode(favoriteAuthors) { UserDefaults.standard.set(d,
                                                                                          forKey: authorsKey) }
    ...
    func loadFromLocal() {
        if let d = UserDefaults.standard.data(forKey: booksKey),
           let v = try? JSONDecoder().decode([Audiobook].self,
                                             from: d)  { favoriteBooks = v }
        if let d = UserDefaults.standard.data(forKey: ebooksKey),
           let v = try? JSONDecoder().decode([Ebook].self,
                                             from: d)      { favoriteEbooks = v }
        if let d = UserDefaults.standard.data(forKey: kidsKey),
           let v = try? JSONDecoder().decode([Ebook].self,
                                             from: d)      { favoriteKidsBooks = v }
        if let d = UserDefaults.standard.data(forKey: authorsKey),
           let v = try? JSONDecoder().decode([Author].self,
                                             from: d)     { favoriteAuthors = v }
...
extension FavoritesViewModel {
    ...
    func syncToFirebase() {
        ...
        guard
            let booksData = try?
                JSONEncoder().encode(favoriteBooks),
            let ebooksData  = try?
                JSONEncoder().encode(favoriteEbooks),
            let kidsData = try?
                JSONEncoder().encode(favoriteKidsBooks),
            let authorsData = try?
                JSONEncoder().encode(favoriteAuthors)
        else { return }
        
        let payload: [String: Any] = [
            "favoriteBooks":
                booksData.base64EncodedString(),
            "favoriteEbooks":
                ebooksData.base64EncodedString(),
            "favoriteKidsBooks":
                kidsData.base64EncodedString(),
            "favoriteAuthors":
                authorsData.base64EncodedString(),
            "favoriteGenres":
                favoriteGenres,
            "updatedAt":
    ...
    func syncFromFirebase() {
        guard let uid else { return }
        db.collection("users").document(uid).getDocument { [weak self] snapshot,
            error in
            guard let self,
